...
if roadmap and roadmap.get("phases"):
    ...
    for ph in roadmap["phases"]:
        ...
        lis = "".join([f"<li><strong>{it.get('skill')}</strong> — {it.get('action')}</li>" for it in items])
        st.markdown(
            f"""
            <div class="phase">
              <span class="badge">Phase {ph.get('phase')}</span>
              <strong> {ph.get('title')}</strong> · {ph.get('duration')}<br>
              <span style="color:#64748b;font-size:0.85rem;">{ph.get('focus')}</span>
              <ul>{lis}</ul>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    # simple fallback plan from missing skills
    if missing:
        for s in missing:
